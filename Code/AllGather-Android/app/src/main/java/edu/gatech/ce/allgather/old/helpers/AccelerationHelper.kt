...
class AccelerationHelper(_context: Context) : HelperClass(_context), SensorEventListener {
    ...
    override fun record(f: File) {
        ...
        if(AllGatherApplication.ENCRYPT_DATA)
        {
            ...
            mCOS.write("timestamp_nanosecond,local_timestamp_milliseconds,accel_x_mps2,accel_y_mps2,accel_z_mps2,rotation_x_sin_theta_by_2,rotation_y_sin_theta_by_2,rotation_z_sin_theta_by_2,rotation_cos_theta_by_2,angvelocity_x_radps,angvelocity_y_radps,angvelocity_z_radps, azu, pitch, roll\n".toByteArray())
        }
        else
        {
            //write orientation and header
            mFOS.write("orientation,${rotation.toString()}\n".toByteArray())
            mFOS.write("timestamp_nanosecond,local_timestamp_milliseconds,accel_x_mps2,accel_y_mps2,accel_z_mps2,rotation_x_sin_theta_by_2,rotation_y_sin_theta_by_2,rotation_z_sin_theta_by_2,rotation_cos_theta_by_2,angvelocity_x_radps,angvelocity_y_radps,angvelocity_z_radps, azu, pitch, roll\n".toByteArray())
        }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (isRecording) {
            try {
                ...
                /* 1/6/2020 Tianqi Liu
                    Output orientation to csv file too
                */
